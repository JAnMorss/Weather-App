...
export default function AdditionalInfo({ coords }: Props) {
    ...
    const currentData = {
      ...
      uvi: data.daily.uv_index_max?.[todayIndex], 
        ...
        >
          {rows.map(({ label, value, Icon }) => (
              <div className="flex justify-between" key={value}>
                ...
                <span>
                  <FormatComponent value={value} number={currentData[value]} />
                </span>
              </div>
          ))}
        </Card>
...
function FormatComponent({ value, number, }: { value: string; number: number | string; }) {
